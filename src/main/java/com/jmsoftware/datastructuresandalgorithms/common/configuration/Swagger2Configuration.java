...
public class Swagger2Configuration {
    ...
    private ApiInfo apiInfo() {
        ...
        return new ApiInfoBuilder()
                ...
                .description(String.format("%s %sArtifact ID: %s%s", projectProperty.getDescription(), LINE_SEPARATOR,
                                           projectArtifactId, LINE_SEPARATOR))
                .contact(new Contact(String.format("%s, email: %s%sHome page: %s", projectProperty.getDeveloperName(),
                                                   developerEmail, LINE_SEPARATOR, developerUrl), developerUrl,
                                     developerEmail))
                .version(version)
                .build();
    }
